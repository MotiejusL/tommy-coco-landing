...
function slide(container, direction, itemsContainer, button) {
  ...
  if (slideWrapper.children.length > 1) slideWrapper.removeChild(slideWrapper.lastChild);
...
function changeItemsMainContNotMobile(itemNodeList, direction, itemsContainer) {
  Array.from(itemNodeList).forEach(function (element, index) {
    if (mobileView.matches) var copyItem = element.cloneNode(true);else {
      if (direction === "left") var copyItem = itemNodeList[mod(index + 1, itemNodeList.length)].cloneNode(true);else if (direction === "right") var copyItem = itemNodeList[mod(index - 1, itemNodeList.length)].cloneNode(true);
    }
...
function setAsideItemsCloneForSlider(itemsCloned, direction) {
  ...
  if (direction === "right") itemsCloned.style.transform = "translateX(-100%)";else if (direction === "left") itemsCloned.style.transform = "translateX(100%)";
}
